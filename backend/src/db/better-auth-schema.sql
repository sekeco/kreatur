create table "user" ("id" text not null primary key, "name" text not null, "email" text not null unique, "emailVerified" boolean not null, "image" text, "twoFactorEnabled" boolean not null default false, "createdAt" timestamptz default CURRENT_TIMESTAMP not null, "updatedAt" timestamptz default CURRENT_TIMESTAMP not null);
...
create table "two_factor" ("id" text not null primary key, "userId" text not null unique references "user" ("id") on delete cascade, "secret" text not null, "backupCodes" text, "verified" boolean, "failedVerificationCount" integer not null default 0, "lockedUntil" timestamptz);
